...
function workerHost() {
  ...
  let offline=false, missing=false, skipped=0, claimed=0, updates=0, build='7-performance';
...
test('offline worker installs the complete versioned shell while bypassing stale HTTP caches',async()=>{
  ...
  assert(html.includes('<meta name="balitopia-build" content="7-performance">'));
...
test('navigation refreshes online and falls back offline, and never returns HTML for a missing script',async()=>{
  const h=workerHost(); await h.dispatch('install');
  const shell=await h.caches.open('balitopia-shell-v7-performance');
  ...
  const js=await h.fetchEvent('./js/gore.js?v=7-performance'); assert.match(await js.text(),/gore.js/);
...
test('a newer HTML entry waits for its matching worker instead of mixing releases',async()=>{
  ...
  assert.match(await old.text(),/content="7-performance"/);
  assert.equal(h.counts().updates,1);
  h.offline(true);
  assert.match(await (await h.fetchEvent('./','navigate')).text(),/content="7-performance"/);
});
