...
function workerHost() {
  ...
  let offline=false, missing=false, skipped=0, claimed=0, updates=0, build='4-red-splatter';
...
test('offline worker installs the complete versioned shell while bypassing stale HTTP caches',async()=>{
  ...
  assert(html.includes('<meta name="balitopia-build" content="4-red-splatter">'));
...
test('navigation refreshes online and falls back offline, and never returns HTML for a missing script',async()=>{
  const h=workerHost(); await h.dispatch('install');
  const shell=await h.caches.open('balitopia-shell-v4-red-splatter');
  ...
  const js=await h.fetchEvent('./js/gore.js?v=4-red-splatter'); assert.match(await js.text(),/gore.js/);
...
test('a newer HTML entry waits for its matching worker instead of mixing releases',async()=>{
  ...
  assert.match(await old.text(),/content="4-red-splatter"/);
  assert.equal(h.counts().updates,1);
  h.offline(true);
  assert.match(await (await h.fetchEvent('./','navigate')).text(),/content="4-red-splatter"/);
});
