...
(async () => {
  ...
  await p.waitForFunction(() => document.body.dataset.ready === '1' && window.__balitopia);
    ...
    for (let rep = 0; rep < reps; rep++) {
      for (const nm of names) {
        ...
        for (let i = 0; i < capN; i++) {
          ...
          const enemy = B.spawnEnemy(i % 7 === 0 ? 'demonder' : 'minyar', i % 5 === 0 ? 1 : 0,
                       pl.x + Math.cos(a) * dd, pl.y + Math.sin(a) * dd);
          if (enemy) enemy.hp = enemy.maxhp = 1e7;
        }
        await new Promise(r => setTimeout(r, 600));
        B.prof(true);
        const t0 = performance.now();
        await new Promise(done => {
          const tick = () => { if (performance.now() - t0 < 2500) requestAnimationFrame(tick); else done(); };
          requestAnimationFrame(tick);
        });
        if (!B.G.running || B.G.over) throw Error('Benchmark stopped simulating');
        const profile = B.profData(); B.prof(false);
        (acc[nm] = acc[nm] || []).push(+(profile.__frames / ((performance.now() - t0) / 1000)).toFixed(1));
        extra[nm] = { view: B.viewInfo(), alive: B.G.aliveEnemies, frame: B.frameInfo(), profile };
      }
    }
    return { acc, extra };
